		...
		if playerPed then
			...
			if daw == true then
				...
				if isWeaponBig(weapon) then
					...
					--if bagIndex == 40 or bagIndex == 41 or bagIndex == 44 or bagIndex == 45 or accessIndex == 11 or accessIndex == 12 or (accessIndex == 4 and GetEntityModel(PlayerPedId()) == GetHashKey("mp_f_freemode_01")) then 
					...
					if GetVehiclePedIsIn(playerPed, false) == 0 and DoesEntityExist(vehicle) and IsEntityAVehicle(vehicle) and bigWeaponOut == false then
						...
						if duffle == true then
							...
							if bagIndex == 40 or bagIndex == 41 or bagIndex == 44 or bagIndex == 45 or accessIndex == 2 or accessIndex == 12 or (accessIndex == 4 and GetEntityModel(PlayerPedId()) == GetHashKey("mp_f_freemode_01")) then 
...
--------------------------------------------------ANIMATION GANGSTER/POLICE ARMES---------------------------------------------------------------
local weapons = {
	'WEAPON_KNIFE',
	'WEAPON_NIGHTSTICK',
	'WEAPON_HAMMER',
...
Citizen.CreateThread(function()
	while true do
		Citizen.Wait(WeaponTiming)

		if IsPedArmed(PlayerPedId(), 4) then
			WeaponTiming = 0
			local newWeap = GetSelectedPedWeapon(PlayerPedId())
			local Coords = GetEntityCoords(PlayerPedId())
			local xCoords = vector3(Coords.x, Coords.y, Coords.z + 0.98)
			loadAnimDict("reaction@intimidation@1h")
			if CheckWeapons(newWeap) then
				if newWeap ~= currWeapon then
					pos = GetEntityCoords(PlayerPedId(), true)
					rot = GetEntityHeading(PlayerPedId())
					if WeapOut then
						TaskPlayAnimAdvanced(PlayerPedId(), "reaction@intimidation@1h", "outro", pos, 0, 0, rot, 8.0, 3.0, -1, 50, 0.125, 0, 0)
						Citizen.Wait(2500)
					end
					SetCurrentPedWeapon(PlayerPedId(), newWeap, true)
					TaskPlayAnimAdvanced(PlayerPedId(), "reaction@intimidation@1h", "intro", pos, 0, 0, rot, 8.0, 3.0, -1, 50, 0.125, 0, 0)
					Citizen.Wait(2500)
