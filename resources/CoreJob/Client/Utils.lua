...
RegisterNetEvent("Refresh:DeathState")
AddEventHandler("Refresh:DeathState", function()
	isDead = false
end)
...
AddEventHandler('esx:onPlayerDeath', function()
	...
	while isDead do
		...
		if CrtTimer >= 0 then
			...
			drawTxt("Appuyer sur ~b~G~s~ pour retouner a l'hopital", 1, {255, 255, 255, 255}, 0.6, 0.4, 0.95)
			if IsControlJustReleased(0, 47) then
				ReviveToHospital()
			end
		end
...
AddEventHandler("Trg:RevivePly", function()
	Coords = GetEntityCoords(PlayerPedId())
	Heading = GetEntityHeading(PlayerPedId())
	isDead = false
	NetworkResurrectLocalPlayer(Coords.x, Coords.y, Coords.z + 1.5, Heading, true, false)
