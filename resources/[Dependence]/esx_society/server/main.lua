...
AddEventHandler('esx_society:washMoney', function(society, amount)
  ...
  local account = xPlayer.getAccount('black_money')
    ...
    local howlong = 120
    local xtime   = 60 -- si 1 = au dessus en SECONDES. si 60 = en MINUTES
    local minorsec  = ''
    if xtime == 1 then
     minorsec = ' secondes'
    ...
    TriggerClientEvent('esx:showNotification', xPlayer.source, ('Tu as ~g~' .. ESX.Math.GroupDigits(amount) .. ' $~s~ en cours de blanchiment ' .. howlong .. minorsec .. '.'))
    Citizen.Wait(1000 * xtime * howlong)TriggerEvent('esx_society:getSociety', xPlayer.job.name, function(society2)
    ...
    TriggerClientEvent('esx:showNotification', xPlayer.source, ('Vos ~g~' .. ESX.Math.GroupDigits(amount) .. ' $~s~ on été Blanchi.'))
