...
RageUI.Settings = {
    ...
    Items = {
        Title = {
            ...
            Text = { X = 65, Y = 0, Scale = 0.5 },
        },
        Subtitle = {
            Background = { Width = 431, Height = 45 },
            Background2 = { Dictionary = "commonmenu", Texture = "gradient_bgd", Y = 4, Width = 431, Height = 45 },
            Text = { X = 215, Y = 40, Scale = 0.7 },
        ...
        Navigation = {
            ...
            Offset = 5,
            Arrows = { Dictionary = "root_cause", Texture = "shop_arrows_upanddown", X = 190, Y = -6, Width = 50, Height = 50 },
        },
        Description = {
            Bar = { Y = 4, Width = 431, Height = 4 },
            Background = { Dictionary = "commonmenu", Texture = "gradient_bgd", Y = 4, Width = 431, Height = 30 },
            Text = { X = 8, Y = 10, Scale = 0.50 },
        },
    },
    Panels = {
        Grid = {
...
function RageUI.Banner()
    ...
    if CurrentMenu ~= nil then
        if CurrentMenu() and (CurrentMenu.Display.Header) then
            ...
            RenderRectangle(CurrentMenu.X, CurrentMenu.Y, RageUI.Settings.Items.Title.Background.Width + CurrentMenu.WidthOffset, RageUI.Settings.Items.Title.Background.Height, MenuColour.Banner.R or 13, MenuColour.Banner.G or 24, MenuColour.Banner.B or 33, MenuColour.Banner.A or 255)
            ...
            DrawScaleformMovie(ScaleformMovie, GlareX, GlareY, Glarewidth / 431, Glareheight / 96.75, 245, 245, 245, 255, 1)
            RenderText("L'as Venturas", CurrentMenu.X + FontPos[MenuFont.SubTitle].Banner.Sub.X + (CurrentMenu.WidthOffset / 2), CurrentMenu.Y + FontPos[MenuFont.SubTitle].Banner.Sub.Y, MenuFont.SubTitle or 6, FontPos[MenuFont.SubTitle].Banner.Sub.Scale, MenuColour.Txt.R or 191, MenuColour.Txt.G or 204, MenuColour.Txt.B or 148, MenuColour.Txt.A or 255, 1)
            RenderText(CurrentMenu.Subtitle, CurrentMenu.X + FontPos[MenuFont.Title].Banner.Title.X + (CurrentMenu.WidthOffset / 2), CurrentMenu.Y + FontPos[MenuFont.Title].Banner.Title.Y, MenuFont.Title or 6, FontPos[MenuFont.Title].Banner.Title.Scale, MenuColour.Txt.R or 191, MenuColour.Txt.G or 204, MenuColour.Txt.B or 148, MenuColour.Txt.A or 255, 1)
    ...
    if CurrentMenu ~= nil then
        if CurrentMenu() and (CurrentMenu.Display.Subtitle) then
            ...
            if CurrentMenu.Subtitle ~= "" then
                ...
                RageUI.ItemOffset = RageUI.ItemOffset + RageUI.Settings.Items.Subtitle.Background.Height - CurrentMenu.SubtitleHeight - 40
            end
        end
    end
end
...
function RageUI.Description()
    ...
    if CurrentMenu ~= nil and CurrentMenu.Description ~= nil then
        if CurrentMenu() and (CurrentMenu.Display.Background) then
            RageUI.ItemsSafeZone(CurrentMenu)
            RenderRectangle(CurrentMenu.X, CurrentMenu.Y + Description.Bar.Y + CurrentMenu.SubtitleHeight + RageUI.ItemOffset , Description.Background.Width + CurrentMenu.WidthOffset, CurrentMenu.DescriptionHeight * 1.3, MenuColour.Desc.R or 13, MenuColour.Desc.G or 24, MenuColour.Desc.B or 33, MenuColour.Desc.A or 150)
            RenderText(CurrentMenu.Description, CurrentMenu.X + Description.Text.X, CurrentMenu.Y + Description.Text.Y + CurrentMenu.SubtitleHeight + RageUI.ItemOffset, MenuFont.Desc or 6, Description.Text.Scale, MenuColour.Txt.R or 191, MenuColour.Txt.G or 204, MenuColour.Txt.B or 148, MenuColour.Txt.A or 255, nil, false, false, Description.Background.Width + CurrentMenu.WidthOffset - 8.0)
