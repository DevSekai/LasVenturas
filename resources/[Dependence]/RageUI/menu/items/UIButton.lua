...
local SettingsButton = {
    ...
    Text = { X = 8, Y = 1, Scale = 0.50 },
    LeftBadge = { Y = -2, Width = 40, Height = 40 },
    RightBadge = { X = 385, Y = -2, Width = 40, Height = 40 },
    RightText = { X = 420, Y = 4, Scale = 0.35 },
    LeftText = { X = 0, Y = 4, Scale = 0.35 },
        ...
        if CurrentMenu.Pagination.Minimum <= Option and CurrentMenu.Pagination.Maximum >= Option then
            ...
            if Enabled then
                ...
                if Style.RightLabel then
                    RenderText(Style.RightLabel, CurrentMenu.X + FontPos[MenuFont.Txt].Txt.X + CurrentMenu.WidthOffset, CurrentMenu.Y + SettingsButton.RightText.Y + CurrentMenu.SubtitleHeight + RageUI.ItemOffset, 0, SettingsButton.RightText.Scale,  MenuColour.Txt.R or 191, MenuColour.Txt.G or 204, MenuColour.Txt.B or 148, MenuColour.Txt.A or 255, 2)
                end
                ...
                --RenderText(Label, CurrentMenu.X + SettingsButton.Text.X + LeftBadgeOffset, CurrentMenu.Y + SettingsButton.Text.Y + RageUI.ItemOffset, 6, SettingsButton.Text.Scale * 2, 245, 245, 245, 255, 1)
            else
                if haveRightBadge then
