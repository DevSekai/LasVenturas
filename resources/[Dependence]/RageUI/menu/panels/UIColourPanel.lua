...
---@type table
local Colour = {
    Background = { Dictionary = "root_cause", Texture = "gradient_bgd", Y = 4, Width = 431, Height = 112 },
    LeftArrow = { Dictionary = "root_cause", Texture = "arrowleft", X = 7.5, Y = 15, Width = 30, Height = 30 },
    RightArrow = { Dictionary = "root_cause", Texture = "arrowright", X = 393.5, Y = 15, Width = 30, Height = 30 },
...
function RageUI.Panel.ColourPanel(Title, Colours, StartedAtIndex, Action, DisplayAtIndex)
    ...
    if CurrentMenu ~= nil then
        if CurrentMenu() and (DisplayAtIndex == nil or (CurrentMenu.Index == DisplayAtIndex)) then
            ...
            RenderRectangle(CurrentMenu.X + Colour.SelectedRectangle.X + (Colour.Box.Width * (CurrentIndex - MinimumIndex)) + (CurrentMenu.WidthOffset / 2), CurrentMenu.Y + Colour.SelectedRectangle.Y + CurrentMenu.SubtitleHeight + RageUI.ItemOffset, Colour.SelectedRectangle.Width, Colour.SelectedRectangle.Height, 0, 0, 0, 255)
