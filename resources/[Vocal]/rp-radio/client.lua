...
Citizen.CreateThread(function()
	...
	if radioConfig.Private[PlayerData.job.name] then
		for _,v in pairs(radioConfig.Private[PlayerData.job.name]) do
			GivePlayerAccessToFrequencies(v)
		end
	end
end)

RegisterNetEvent('AddPoliceRadio')
AddEventHandler('AddPoliceRadio', function()
	ESX.ShowNotification("Vous avez pirater les radio de la police.")
	for _,v in pairs(radioConfig.Private["police"]) do
		ESX.ShowNotification("[~b~Fréquence~s~] : "..v.." MHz")
		GivePlayerAccessToFrequencies(v)
	end
end)
...
AddEventHandler('esx:setJob', function(job)
	if radioConfig.Private[LastJob] then
		for _,v in pairs(radioConfig.Private[LastJob]) do
