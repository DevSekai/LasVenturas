		...
		local elements2 = {
		  ...
		  function(data2, menu2)
			...
			if data2.current.value == 'sim_use' then
				...
				ESX.ShowNotification("Vous avez ~g~activé~s~ la carte SIM avec le numéro : ~p~" .. data.current.value.number)
			...
			if data2.current.value == 'sim_delete' then
				...
				ESX.ShowNotification("Vous avez ~r~jeté~s~ la carte SIM avec le numéro : ~p~" .. data.current.value.number)
				TriggerServerEvent('gcPhone:allUpdate')
				TriggerServerEvent("gcPhone:myPhoneNumber", sourcePlayer, myPhoneNumber)
			end
...
function OpenPhoneMenu()
	...
	local elements = {
	  {label = 'Mes cartes SIM', value = 'sim_phone'},
	  {label = 'Actualiser le téléphone', value = 'sim_actualise'}
	  ...
	  function(data, menu)
		...
		if data.current.value == 'open_phone' then
			...
			ESX.TriggerServerCallback('esx_cartesim:CheckSim', function(sim)
				if sim then
					TriggerEvent('gcphone:OpenZ')
				else
					ESX.ShowNotification("~r~Vous n'avez pas de carte SIM !")
				end
			end)

		end

		if data.current.value == 'sim_actualise' then
			ESX.UI.Menu.CloseAll()
			ESX.ShowNotification("Vous avez ~g~actualisé~s~ votre téléphone")
				TriggerServerEvent('gcPhone:allUpdate')
				TriggerServerEvent("gcPhone:myPhoneNumber", sourcePlayer)
		end
